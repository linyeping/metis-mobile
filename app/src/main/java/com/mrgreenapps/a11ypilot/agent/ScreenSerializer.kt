...
object ScreenSerializer {
    ...
    /**
     * Computes a compact diff between [prev] and [curr], returning only the lines that changed
     * plus the new screen's header. When the two frames are mostly identical (the common case
     * after a single tap), this can cut the per-turn screen payload by a large fraction, saving
     * input tokens on every agent loop iteration.
     *
     * The returned string is a *delta* view; callers must tell the model that unchanged lines are
     * carried over from the previous screen.
     */
    fun diff(prev: Snapshot?, curr: Snapshot): String {
        if (prev == null) return curr.text
        val prevLines = prev.text.lineSequence().toList()
        val currLines = curr.text.lineSequence().toList()
        // Header lines (foreground/screen) are always emitted.
        val header = currLines.take(2)
        val prevBody = prevLines.drop(2)
        val currBody = currLines.drop(2)
        val changed = currBody.filterNot { it in prevBody }
        if (changed.isEmpty()) {
            return (header + listOf("(no visible change)")).joinToString("\n")
        }
        return (header + changed).joinToString("\n")
    }

...
class McpService : android.app.Service() {
    ...
    private var relay: McpRelayClient? = null
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        ...
        // Launch a coroutine to get the token
        CoroutineScope(Dispatchers.IO + SupervisorJob()).launch {
            ...
            // Optional outbound relay: reach the phone's MCP from outside the LAN via a configured
            // forwarder, without exposing 0.0.0.0 publicly.
            val snap = try { AgentSettings.snapshot(ctx) } catch (t: Throwable) { null }
            val relayUrl = snap?.mcpRelayUrl.orEmpty()
            val relayToken = snap?.mcpRelayToken.orEmpty()
            if (relayUrl.isNotBlank() && relayToken.isNotBlank() && relay == null) {
                val executor = com.mrgreenapps.a11ypilot.agent.ToolExecutor(ctx, excludeOwnPackage = true)
                relay = McpRelayClient(relayUrl, relayToken, JsonRpc(executor)).also {
                    EventLog.append("mcp> starting relay $relayUrl")
                    it.start()
                }
            }
        }
    ...
    override fun onDestroy() {
        super.onDestroy()
        try { relay?.stop() } catch (_: Throwable) {}
        relay = null
